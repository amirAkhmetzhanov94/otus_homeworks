...
from src.app.log_file_helpers import (
    collect_log_file_names,
    extract_log_data,
    find_log_file_name_and_date,
    get_latest_log_file,
    validate_log_file_names,
)


def test_extract_log_data_valid() -> None:
    log_line = (
...
def test_extract_log_data_invalid() -> None:
...
@patch("os.listdir")
def test_collect_log_file_names(mock_listdir: MagicMock) -> None:
    mock_listdir.return_value = [
        "nginx-access-ui.log-20231010",
        "nginx-access-ui.log-20231009.gz",
    ]
...
@patch("os.listdir")
def test_collect_log_file_names_empty_directory(mock_listdir: MagicMock) -> None:
    mock_listdir.return_value = []
    log_folder = "/path/to/logs"
    result = collect_log_file_names(log_folder)
    assert result == []
...
def test_validate_log_file_names_valid() -> None:
    log_file_names = [
        "nginx-access-ui.log-20231010.gz",
        "nginx-access-ui.log-20231009.gz",
        "other-file.txt",
    ]
...
def test_validate_log_file_names_invalid() -> None:
    log_file_names = [
        "nginx-access-ui.log-20231010.gz",
        "invalid-log-file",
        "other-file.txt",
...
def test_get_latest_log_file() -> None:
    log_folder = "/path/to/logs"
...
def test_find_log_file_name_and_date(
    mock_get_latest_log_file: MagicMock,
    mock_validate_log_file_names: MagicMock,
    mock_collect_log_file_names: MagicMock,
) -> None:
    ...
    assert result == (
        "/path/to/logs/nginx-access-ui.log-20231010",
        "20231010.gz",
    )
...
@patch("src.app.log_file_helpers.collect_log_file_names")
@patch("src.app.log_file_helpers.validate_log_file_names")
def test_find_log_file_name_and_date_no_logs(
    mock_validate_log_file_names: MagicMock,
    mock_collect_log_file_names: MagicMock,
) -> None:
    config = {"LOG_DIR": "/path/to/logs"}
