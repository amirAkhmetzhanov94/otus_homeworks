...
def test_generate_percentiles_report_valid():
    requests_data = {
        "url1": {"count": 10, "times": [1.0, 2.0, 3.0]},
        "url2": {"count": 5, "times": [4.0, 5.0]},
    }
    total_count = 15
    result = generate_percentiles_report(requests_data, "count", total_count)
    assert result == {"url1": 66.66666666666666, "url2": 33.33333333333333}


def test_generate_percentiles_report_with_lists():
    requests_data = {
...
def test_generate_percentiles_report_zero_total_count():
...
def test_collect_request_data_valid():
    log_data = [("url1", "1.0"), ("url2", "2.0"), ("url1", "3.0")]
...
def test_collect_request_data_empty_log_data():
    log_data = []
    result = collect_request_data(log_data)
    assert result == {}


def test_generate_report_data_valid():
    time_stats = {
    ...
    result = generate_report_data(time_stats, requests_data, time_percentiles, count_percentiles)
    expected_result = [
        {
            "count": 2,
            "url": "url1",
...
def test_generate_report_file_name_valid():
    file_date = "20231010"
    result = generate_report_file_name(file_date)
    assert result == "/report-2023.10.10.html"


def test_generate_report_file_name_invalid_format():
    file_date = "202310"  # Missing day
    result = generate_report_file_name(file_date)
    assert result == "/report-2023.10.31.html"


def test_generate_reports_valid(mocker):
    mock_log_lines = [
        '127.0.0.1 - - [10/Oct/2023:13:55:36 +0000] "GET /url1 HTTP/1.1" 200 1234 "-" "curl/7.64.1" 1.0',
        '127.0.0.1 - - [10/Oct/2023:13:55:37 +0000] "GET /url2 HTTP/1.1" 200 1234 "-" "curl/7.64.1" 2.0',
        '127.0.0.1 - - [10/Oct/2023:13:55:38 +0000] "GET /url1 HTTP/1.1" 200 1234 "-" "curl/7.64.1" 3.0',
    ]
